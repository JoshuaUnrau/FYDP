...
class Motor_Channel(IntEnum):
	front_right_forward = 12
	front_right_backward = 13
	rear_right_forward = 14
	rear_right_backward = 15
...
class PWM_Driver:
	def __init__(self):
		...
		self.LADirection = ActuatorState.Stop
	...
	def check_linear_actuator(self):
		...
		# Up
		if self.LADirection == ActuatorState.Up:
		...
		# Zero
		if self.LADirection == ActuatorState.Stop:
			up_channel.duty_cycle = 0
			down_channel.duty_cycle = 0
			if self.limitSwitchState == LimitSwitchState.Up:
				self.actuatorState = ActuatorState.Up
...
def limit_switch_callback(state):
	...
	pwm_driver.check_linear_actuator()
